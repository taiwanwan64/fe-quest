...
req("const APP_VERSION = 'v157';" in h and 'runV157SelfCheck();' in h,'version/boot')
req('"name": "FE QUEST v157"' in manifest,'manifest')
req("const APP_VERSION = 'v157';" in sw and "fe-quest-v157-1" in sw,'sw')
...
req(sha(proj)==ds['projection']['sha256']=='88db821278597a5a2dc073da6935ceb979b39632b243fed9cd7846cd924abe50' and len(proj.encode())==55525,'projection')
...
print('FEQUEST_V157_FIXTURE_BOUNDARY_OK production=excluded fixture=293 data=6 backing=0')
class P(HTMLParser):
...
print('FEQUEST_V157_STATIC_DOM_OK 23/23 + required-dom')
...
checks="""if(APP_VERSION!=='v157')throw Error('version');const s=FEQUEST_SELF_CHECK;if(!s||!s.ok||s.currentContract.total!==71||s.currentContract.passed!==71||s.architecture!=='v157-stable-semantic-diagnostic-data-runtime')throw Error('self');if(s.browserUiContract.total!==23)throw Error('ui');if(s.semanticRuntimeBoundary.stable!==17||s.semanticRuntimeBoundary.retired!==46||!s.semanticRuntimeBoundary.ok)throw Error('helpers');if(s.semanticDataBoundary.semantic!==6||s.semanticDataBoundary.leakedBacking.length||!s.semanticDataBoundary.frozen)throw Error('data');if(QUESTION_BANK.length!==710||new Set(QUESTION_BANK.map(q=>q.id)).size!==710)throw Error('q');if([0,1,2,3].map(i=>QUESTION_BANK.filter(q=>q.a===i).length).join(',')!=='178,178,177,177')throw Error('a');if(['想起','適用','判断'].map(k=>QUESTION_BANK.filter(q=>q.cognitiveLevel===k).length).join(',')!=='166,323,221')throw Error('cog');if(s.declarativeCiCoverage.total!==84||s.declarativeCiCoverage.critical!==56||s.declarativeCiCoverage.release!==28)throw Error('ci');console.log('FEQUEST_V157_PRODUCTION_RUNTIME_OK current=71/71 stable=17 retired-fn=46 data=6 backing=0 critical-map=56 release-map=28 ci=84 legacy-bundled=0');"""
...
diag=json.loads(Path('_regression/diagnostic-helper-boundary-v154.fixture.json').read_text()); dsources=''.join(Path(x['path']).read_text() for x in diag['source_blocks']); tmpl=legacy['release_shell_template']; adapted=fixture.replace(legacy['release_shell_from'],tmpl.replace('{{VERSION}}','v157'))
rel="""const cc=runV150CriticalCurriculumAudit(),rs=runV151ReleaseSentinelAudit(),l=runV149LegacyShadowAudit();if(cc.total!==56||cc.failed||rs.total!==28||rs.failed||l.rawErrorCount!==22||l.residualActiveErrors.length||(String(runAppSelfCheck).match(/\\bassert\\s*\\(/g)||[]).length!==293)throw Error('release');console.log('FEQUEST_V157_RELEASE_FIXTURE_OK diagnostic=46 critical=56/56 release=28/28 legacy=293 raw=22 residual=0');"""
...
print('FEQUEST_V157_RUNTIME_OK current=71/71 stable=17 retired-fn=46 data=6 backing=0 critical=56/56 release=28/28 ci=84 legacy=293 residual=0 production-legacy=0')
