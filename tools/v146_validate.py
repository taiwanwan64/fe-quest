...
print('FEQUEST_V146_STATIC_DOM_OK 13/13')

scripts=re.findall(r'<script(?:\s[^>]*)?>(.*?)</script>',html,re.S|re.I)
js='\n'.join(s for s in scripts if s.strip() and not s.lstrip().startswith('{'))
Path('/tmp/fe-v146-inline.js').write_text(js,encoding='utf-8')
subprocess.run(['node','--check','/tmp/fe-v146-inline.js'],check=True)
...
'''
checks=r'''
if(APP_VERSION!=='v146')throw new Error(`version ${APP_VERSION}`);
...
if(!sc||sc.architecture!=='v146-expanded-current-contract'||!sc.ok||(sc.errors||[]).length)throw new Error(`self check ${JSON.stringify(sc?.errors)}`);
if(sc.currentContract?.total!==38||sc.currentContract?.passed!==38||sc.currentContract?.failed!==0)throw new Error(`current ${JSON.stringify(sc?.currentContract)}`);
const inv=sc.v146MigrationInventory;
if(!inv||inv.inheritedCurrentContracts!==23||inv.newlyMigratedGroups!==15||inv.migratedLegacyAssertionEquivalents!==62||inv.legacyShadowAssertCalls!==293||inv.compatibilityResidualCount!==0||inv.browserUiContracts!==13)throw new Error(`inventory ${JSON.stringify(inv)}`);
...
if(sc.v146RuntimeMode!=='node-minimal-dom'||sc.v146DiagnosticContract?.activeErrorCount!==0)throw new Error('runtime mode');
console.log('FEQUEST_V146_RUNTIME_OK');
'''
runner=Path('/tmp/fe-v146-runtime.js');runner.write_text(stub+'\n'+js+'\n'+checks,encoding='utf-8')
r=subprocess.run(['node',str(runner)],capture_output=True,text=True)
print(r.stdout);print(r.stderr,file=sys.stderr)
if r.returncode!=0 or 'FEQUEST_V146_RUNTIME_OK' not in r.stdout:raise SystemExit(r.returncode or 1)
