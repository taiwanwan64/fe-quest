...
req('window.FEQUEST_SELF_CHECK=runV154SelfCheck();' in h,'v154-self-check-boot-missing')
req("const APP_VERSION = 'v154';" in h,'v154-version-missing')
manifest=Path('_site/manifest.webmanifest').read_text(encoding='utf-8')
sw=Path('_site/sw.js').read_text(encoding='utf-8')
req('"name": "FE QUEST v154"' in manifest,'manifest-version')
req("const APP_VERSION = 'v154';" in sw and "const CACHE_NAME = 'fe-quest-v154-1';" in sw,'service-worker-version')
...
req(len(diag_spec['production_modules'])==8,'diagnostic-runtime-module-count')
for path in diag_spec['production_modules']:
    req(Path(path).is_file(),'diagnostic-runtime-module-missing:'+path)
...
print('FEQUEST_V154_FIXTURE_BOUNDARY_OK production=excluded fixture=293')
class N:
...
print('FEQUEST_V154_STATIC_DOM_OK 23/23 + required-dom')

scripts=re.findall(r'<script(?:\s[^>]*)?>(.*?)</script>',h,re.S|re.I)
js='\n'.join(s for s in scripts if s.strip() and not s.lstrip().startswith('{'))
Path('/tmp/fequest_v154_prod.js').write_text(js,encoding='utf-8')
subprocess.run(['node','--check','/tmp/fequest_v154_prod.js'],check=True)
...
prod_checks=r"""if(APP_VERSION!=='v154')throw Error('version');
if(typeof runAppSelfCheck!=='undefined')throw Error('legacy evaluator bundled');
const runtimeNames=V154_DIAGNOSTIC_BOUNDARY_SPEC.productionRuntime;
const prunedNames=[...V154_DIAGNOSTIC_BOUNDARY_SPEC.releaseOnly,...V154_DIAGNOSTIC_BOUNDARY_SPEC.compatibilityOnly];
if(runtimeNames.length!==17||prunedNames.length!==29||runtimeNames.some(n=>typeof globalThis[n]!=='function')||prunedNames.some(n=>typeof globalThis[n]!=='undefined'))throw Error('diagnostic physical pruning');
if(typeof runV153SelfCheck!=='undefined'||typeof v153EvaluateCurrentContract!=='undefined')throw Error('v153 compatibility layer bundled');
...
if(!s||!s.ok||s.architecture!=='v154-physical-diagnostic-helper-pruning'||s.currentContract.total!==71||s.currentContract.passed!==71||s.currentContract.failed)throw Error('self');
...
const db=s.v154DiagnosticProductionBoundary;
if(!db||!db.ok||db.total!==46||db.unique!==46||db.productionRuntime!==17||db.releaseOnly!==10||db.compatibilityOnly!==19||db.presentRuntime!==17||db.prunedFromProduction!==29||db.leaked.length)throw Error('boundary');
...
console.log('FEQUEST_V154_PRODUCTION_RUNTIME_OK current=71/71 helpers=46 runtime=17 pruned=29 critical-map=56 release-map=28 ci=84 legacy-bundled=0');"""

prod_script=Path('/tmp/fequest_v154_prod_runtime.js')
...
req(z.returncode==0 and 'FEQUEST_V154_PRODUCTION_RUNTIME_OK' in z.stdout,'production-runtime')

# Release-only compatibility validation: load the removed diagnostic fixture only after production self-check,
# then load the historical 293-assert source-range fixture and run all release executors.
template=spec.get('release_shell_template')
req(template and '{{VERSION}}' in template,'fixture-release-shell-template')
fixture_release=fixture.replace(spec['release_shell_from'],template.replace('{{VERSION}}','v154'))
req(fixture_release!=fixture,'fixture-release-shell-adapter')
release_checks=r"""const runtimeNames=V154_DIAGNOSTIC_BOUNDARY_SPEC.productionRuntime;
const prunedNames=[...V154_DIAGNOSTIC_BOUNDARY_SPEC.releaseOnly,...V154_DIAGNOSTIC_BOUNDARY_SPEC.compatibilityOnly];
if(prunedNames.some(n=>typeof globalThis[n]!=='function'))throw Error('diagnostic fixture helpers missing');
...
console.log('FEQUEST_V154_RELEASE_FIXTURE_OK diagnostic=29 critical=56/56 release=28/28 legacy=293 raw=22 residual=0');"""
legacy_script=Path('/tmp/fequest_v154_fixture_runtime.js')
...
req(q.returncode==0 and 'FEQUEST_V154_RELEASE_FIXTURE_OK' in q.stdout,'fixture-runtime')

print('FEQUEST_V154_RUNTIME_OK current=71/71 helpers=46 runtime=17 pruned=29 critical=56/56 release=28/28 ci=84 legacy=293 residual=0 production-legacy=0')
