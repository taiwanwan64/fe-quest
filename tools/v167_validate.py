...
from v167_runtime_stub import STUB
...
req("const APP_VERSION = 'v167';" in h and 'runV167SelfCheck();' in h,'version/boot')
req('"name": "FE QUEST v167"' in manifest,'manifest')
req("const APP_VERSION = 'v167';" in sw and "fe-quest-v167-1" in sw,'sw-version')
...
# v167 archive/source boundary.
fixture=json.loads(Path('_regression/production-source-archive-boundary-v167.fixture.json').read_text())
req(fixture['version']=='v167' and fixture['archived_source_count']==49 and len(fixture['archive_entries'])==49,'archive count')
...
# App source boundary: historical diagnostic/versioned sources remain outside app/.
residual=[]
for p in Path('app').iterdir():
    n=p.name
    if re.fullmatch(r'v(?:14[5-9]|15\d|16[0-6])-block-\d\d\.txt',n): residual.append(n)
...
req(Path('app/v167-block-00.txt').exists(),'v167 adapter missing')
req(not Path('app/v166-block-00.txt').exists(),'v166 adapter still in app')
req('{% include_relative app/v167-block-00.txt %}' in src and 'app/v166-block-00.txt' not in src,'assembler adapter')
...
# Stable semantic runtime remains byte-identical to v165/v166.
...
for key in ['stable_wrapper','release_adapter','assembler','manifest','service_worker','patch_chain_fixture']:
    d=fixture[key]; req(sha_file(d['path'])==d['sha256'],f'fixture identity {key}')

# Stable wrapper tracks the new archive boundary and retired v166 adapter.
w=Path('app/runtime-diagnostic-wrapper.txt').read_text()
req("archiveBoundaryFixture:'_regression/production-source-archive-boundary-v167.fixture.json'" in w and "archiveRoot:'_regression/archive/diagnostics'" in w and 'archivedSourceCount:49' in w,'wrapper archive metadata')
req("retiredReleaseAdapters:Object.freeze(['runV160SelfCheck','runV161SelfCheck','runV162SelfCheck','runV163SelfCheck','runV164SelfCheck','runV165SelfCheck','runV166SelfCheck'])" in w,'retired adapter inventory')
...
# Declarative inventory of the active v132-v144 learning patch chain.
...
req(patch['assembler']['patch_include_count']==47,'patch include count')
concat=b''
for r in rows:
...
# Inventory-only release: the learner patch chain, base, and stable semantic runtime must not differ from main.
...
req(not forbidden,'inventory-only source changed: '+','.join(forbidden))

# Immutable legacy evaluator fixture.
...
print('FEQUEST_V167_STATIC_DOM_OK 23/23 + required-dom')

# Syntax + production runtime smoke.
...
checks="""if(APP_VERSION!=='v167')throw Error('version');const s=FEQUEST_SELF_CHECK;if(!s||!s.ok||s.currentContract.total!==71||s.currentContract.passed!==71||s.architecture!=='stable-semantic-diagnostic-wrapper'||s.releaseVersion!=='v167'||s.releaseAdapter!=='runV167SelfCheck')throw Error('self');if(s.browserUiContract.total!==23)throw Error('ui');if(s.semanticRuntimeBoundary.stable!==17||s.semanticRuntimeBoundary.retired!==46||s.semanticRuntimeBoundary.stableWrapper!==6||s.semanticRuntimeBoundary.retiredWrapper!==6||s.semanticRuntimeBoundary.retiredAdapters!==7||s.semanticRuntimeBoundary.presentStableWrapper!==6||s.semanticRuntimeBoundary.leakedRetiredWrapper.length||s.semanticRuntimeBoundary.leakedRetiredAdapters.length||!s.semanticRuntimeBoundary.ok)throw Error('wrapper');if(s.semanticDataBoundary.semantic!==6||s.semanticDataBoundary.leakedBacking.length||!s.semanticDataBoundary.frozen)throw Error('data');if(typeof runV167SelfCheck!=='function'||typeof runV166SelfCheck!=='undefined'||typeof runV165SelfCheck!=='undefined'||typeof runV164SelfCheck!=='undefined'||typeof runV163SelfCheck!=='undefined'||typeof runV162SelfCheck!=='undefined'||typeof runV161SelfCheck!=='undefined'||typeof runV160SelfCheck!=='undefined')throw Error('adapters');if(QUESTION_BANK.length!==710||new Set(QUESTION_BANK.map(q=>q.id)).size!==710)throw Error('q');if([0,1,2,3].map(i=>QUESTION_BANK.filter(q=>q.a===i).length).join(',')!=='178,178,177,177')throw Error('a');if(['想起','適用','判断'].map(k=>QUESTION_BANK.filter(q=>q.cognitiveLevel===k).length).join(',')!=='166,323,221')throw Error('cog');if(s.declarativeCiCoverage.total!==84||s.declarativeCiCoverage.critical!==56||s.declarativeCiCoverage.release!==28)throw Error('ci');if(FEQ_DIAGNOSTIC_RUNTIME_SPEC.archiveBoundaryFixture!=='_regression/production-source-archive-boundary-v167.fixture.json'||FEQ_DIAGNOSTIC_RUNTIME_SPEC.archiveRoot!=='_regression/archive/diagnostics'||FEQ_DIAGNOSTIC_RUNTIME_SPEC.archivedSourceCount!==49)throw Error('archive-spec');console.log('FEQUEST_V167_PRODUCTION_RUNTIME_OK current=71/71 stable=17 retired-fn=46 wrapper=6 retired-wrapper=0 retired-adapter=0 retired-adapter-inventory=7 adapter=1 data=6 backing=0 archive=49 app-residual=0 patches=47 critical-map=56 release-map=28 ci=84 legacy-bundled=0');"""
...
# Release-only historical helpers execute from the build-excluded archive.
...
tmpl=legacy['release_shell_template']; adapted=legacy_src.replace(legacy['release_shell_from'],tmpl.replace('{{VERSION}}','v167'))
rel="""const cc=runV150CriticalCurriculumAudit(),rs=runV151ReleaseSentinelAudit(),l=runV149LegacyShadowAudit();if(cc.total!==56||cc.failed||rs.total!==28||rs.failed||l.rawErrorCount!==22||l.residualActiveErrors.length||(String(runAppSelfCheck).match(/\\bassert\\s*\\(/g)||[]).length!==293)throw Error('release');console.log('FEQUEST_V167_RELEASE_FIXTURE_OK diagnostic=46 critical=56/56 release=28/28 legacy=293 raw=22 residual=0 archive-source=8');"""
...
print('FEQUEST_V167_PATCH_INVENTORY_OK versions=13 blocks=47 assembler=47 base=1 unchanged-learning-source=1')
print('FEQUEST_V167_ARCHIVE_BOUNDARY_OK archive=49 app-residual=0 deployed-archive=0 runtime=55525')
print('FEQUEST_V167_RUNTIME_OK current=71/71 stable=17 retired-fn=46 wrapper=6 retired-wrapper=0 retired-adapter=0 retired-adapter-inventory=7 adapter=1 data=6 backing=0 archive=49 app-residual=0 patches=47 critical=56/56 release=28/28 ci=84 legacy=293 residual=0 production-legacy=0')
