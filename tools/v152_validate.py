...
req('window.FEQUEST_SELF_CHECK=runV152SelfCheck();' in h,'v152-self-check-boot-missing')
req("const APP_VERSION = 'v152';" in h,'v152-version-missing')
manifest=Path('_site/manifest.webmanifest').read_text(encoding='utf-8')
sw=Path('_site/sw.js').read_text(encoding='utf-8')
req('"name": "FE QUEST v152"' in manifest,'manifest-version')
req("const APP_VERSION = 'v152';" in sw and "const CACHE_NAME = 'fe-quest-v152-1';" in sw,'service-worker-version')
req(not any(Path('_site').rglob('legacy-run-app-self-check-v131.fixture.json')),'legacy-fixture-spec-deployed')
print('FEQUEST_V152_FIXTURE_BOUNDARY_OK production=excluded fixture=293')
...
print('FEQUEST_V152_STATIC_DOM_OK 23/23 + required-dom')

scripts=re.findall(r'<script(?:\s[^>]*)?>(.*?)</script>',h,re.S|re.I)
js='\n'.join(s for s in scripts if s.strip() and not s.lstrip().startswith('{'))
Path('/tmp/fequest_v152_prod.js').write_text(js,encoding='utf-8')
subprocess.run(['node','--check','/tmp/fequest_v152_prod.js'],check=True)
...
prod_checks=r"""if(APP_VERSION!=='v152')throw Error('version');
...
if(!s||!s.ok||s.architecture!=='v152-production-legacy-fixture-boundary'||s.currentContract.total!==69||s.currentContract.passed!==69||s.currentContract.failed)throw Error('self');
if(s.historicalDiagnostics.mode!=='deferred-to-ci'||s.historicalDiagnostics.executed!==false)throw Error('defer');
...
const inv=s.v149LegacyInventory,b=inv.bucketCounts;
if(inv.sourceAssertCalls!==0||inv.classifiedCalls!==293||inv.uniqueClassifiedCalls!==293||inv.duplicateCount||!inv.exactCoverage||!inv.declaredCountsOk||b.currentContractCovered!==147||b.browserContractCovered!==24||b.historicalMetadata!==38||b.ciOnlyLegacySentinels!==84)throw Error('legacy inventory');
...
const fp=s.v152LegacyFixturePolicy;
if(!fp||fp.productionFunctionPresent!==false||fp.productionSourceAssertCalls!==0||fp.releaseFixtureAssertions!==293||fp.releaseFixtureSha256!=='b7f6a3eea1e5c609844311ba9acfe17179df19e07b9c82354ff9ee87c6922f36'||fp.releaseCiExecutesLegacy!==true||fp.productionExecutesLegacy!==false)throw Error('fixture policy');
...
console.log('FEQUEST_V152_PRODUCTION_RUNTIME_OK current=69/69 critical=56/56 release=28/28 ci=84 legacy-bundled=0');"""

prod_script=Path('/tmp/fequest_v152_prod_runtime.js')
...
req(z.returncode==0 and 'FEQUEST_V152_PRODUCTION_RUNTIME_OK' in z.stdout,'production-runtime')

# Release-only compatibility shadow: emulate the historical assembler's one release-shell substitution,
# then load the fixture after the production bundle has already completed its own v152 self-check.
fixture_release=fixture.replace(spec['release_shell_from'],spec['release_shell_to_v152'])
...
console.log('FEQUEST_V152_RELEASE_FIXTURE_OK legacy=293 raw=22 residual=0');"""
legacy_script=Path('/tmp/fequest_v152_fixture_runtime.js')
...
req(q.returncode==0 and 'FEQUEST_V152_RELEASE_FIXTURE_OK' in q.stdout,'fixture-runtime')

print('FEQUEST_V152_RUNTIME_OK current=69/69 critical=56/56 release=28/28 ci=84 legacy=293 residual=0 production-legacy=0')
