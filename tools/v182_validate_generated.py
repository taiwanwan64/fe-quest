...
from v182_runtime_stub import STUB
...
FIXTURE=Path('_regression/steady-state-diagnostic-architecture-v182.fixture.json')
AUDIT=Path('audits/STEADY_STATE_DIAGNOSTIC_ARCHITECTURE_AUDIT_v182.txt')
INV=Path('_regression/diagnostic-archive-inventory.fixture.json')
FORBIDDEN=Path('_regression/production-source-archive-boundary-v182.fixture.json')
...
for p,b,s in stable:
    ...
    req(q.read_bytes()==subprocess.check_output(['git','show',PARENT+':'+p]),'v181 parent byte drift '+p)

spec=Path('app/runtime-release-diagnostic-spec.txt').read_text()
req('const releaseVersion=APP_VERSION;' in spec,'stable metadata version source')
req(not re.search(r"['\"]v182['\"]",spec),'v182 literal embedded in stable metadata')
req(not re.search(r"['\"]runV\d+SelfCheck['\"]",spec),'adapter literal embedded in stable metadata')
req(not FORBIDDEN.exists(),'v182 release-specific boundary exists')
...
prod_path=Path('_site/index.html'); req(prod_path.exists(),'real Jekyll output missing')
prod_b=prod_path.read_bytes(); prod=prod_b.decode(); src=Path('index.html').read_text()
req("const APP_VERSION = 'v182';" in prod and '<title>FE QUEST PWA v182</title>' in prod,'v182 generated shell')
req("globalThis['runV'+APP_VERSION.slice(1)+'SelfCheck']()" in prod,'dynamic boot missing')
...
req('"name": "FE QUEST v182"' in Path('_site/manifest.webmanifest').read_text(),'manifest v182')
sw=Path('_site/sw.js').read_text(); req("const APP_VERSION = 'v182';" in sw and 'fe-quest-v182-1' in sw,'SW v182')
...
print('FEQUEST_V182_STATIC_DOM_OK 23/23 + required-dom')

retired='||'.join(f"typeof globalThis.runV{v}SelfCheck!=='undefined'" for v in range(160,182))
CHECKS=f'''const s=globalThis.FEQUEST_SELF_CHECK;if(APP_VERSION!=='v182'||!s||!s.ok||s.currentContract.total!==71||s.currentContract.passed!==71||s.browserUiContract.total!==23||s.releaseVersion!=='v182'||s.releaseAdapter!=='runV182SelfCheck')throw Error('self '+JSON.stringify(s?.errors));if(s.semanticRuntimeBoundary.stable!==17||s.semanticRuntimeBoundary.retired!==46||s.semanticRuntimeBoundary.stableWrapper!==6||s.semanticRuntimeBoundary.retiredAdapters!==22||s.semanticRuntimeBoundary.leakedRetiredAdapters.length||!s.semanticRuntimeBoundary.ok)throw Error('wrapper boundary');if(globalThis.FEQ_DIAGNOSTIC_RUNTIME_SPEC.archiveBoundaryFixture!=='_regression/diagnostic-archive-inventory.fixture.json'||globalThis.FEQ_DIAGNOSTIC_RUNTIME_SPEC.archivedSourceCount!==58)throw Error('archive projection');if(typeof globalThis.FEQ_RELEASE_DIAGNOSTIC_SPEC!=='undefined')throw Error('metadata global leak');if(typeof globalThis.runV182SelfCheck!=='function'||{retired})throw Error('adapter boundary');if(QUESTION_BANK.length!==710||new Set(QUESTION_BANK.map(q=>q.id)).size!==710)throw Error('questions');if([0,1,2,3].map(i=>QUESTION_BANK.filter(q=>q.a===i).length).join(',')!=='178,178,177,177')throw Error('answers');if(['想起','適用','判断'].map(k=>QUESTION_BANK.filter(q=>q.cognitiveLevel===k).length).join(',')!=='166,323,221')throw Error('cognitive');if(s.declarativeCiCoverage.total!==84||s.declarativeCiCoverage.critical!==56||s.declarativeCiCoverage.release!==28)throw Error('CI coverage');if(s.legacyClassification.total!==293||s.legacyClassification.unique!==293||!s.legacyClassification.exactCoverage)throw Error('legacy classification');'''
SNAP=r'''
...
print('FEQUEST_V182_PRODUCTION_RUNTIME_OK current=71/71 stable=17 retired-fn=46 wrapper=6 retired-adapter=0 retired-adapter-inventory=22 adapter=1 data=6 backing=0 diagnostic-archive=58 archive-growth=0 stable-archive-inventory=1 release-boundary-created=0 learning-archive=48 base-archive=2 active-learning=1 active-base=1 stable-adapter=1 stable-release-metadata=1 versioned-adapter-source=0 diagnostic-architecture-changed=0 critical-map=56 release-map=28 ci=84 legacy-bundled=0')
...
adapted=legacy.replace(legacy_fx['release_shell_from'],legacy_fx['release_shell_template'].replace('{{VERSION}}','v182'))
rel=r'''const cc=runV150CriticalCurriculumAudit(),rs=runV151ReleaseSentinelAudit(),l=runV149LegacyShadowAudit();if(cc.total!==56||cc.failed||rs.total!==28||rs.failed||l.rawErrorCount!==22||l.residualActiveErrors.length||(String(runAppSelfCheck).match(/\bassert\s*\(/g)||[]).length!==293)throw Error('release');console.log('FEQUEST_V182_RELEASE_FIXTURE_OK diagnostic=46 critical=56/56 release=28/28 legacy=293 raw=22 residual=0 archive-source=8');'''
with tempfile.TemporaryDirectory() as td:
...
fx['validation']={
 ...
 'wrapper_byte_identical_to_v181_parent':True,
 'stable_adapter_byte_identical_to_v181_parent':True,
 'stable_release_metadata_byte_identical_to_v181_parent':True,
 'stable_inventory_byte_identical_to_v181_parent':True,
 'release_specific_diagnostic_architecture_changed_files':0,
 'diagnostic_archive_growth':0,
 'release_specific_v182_archive_boundary_created':False,
...
AUDIT.write_text(f'''FE QUEST v182 — Steady-State Diagnostic Architecture Audit\n===========================================================\n\nResult\n------\nPASSED authoritative GitHub Pages/Jekyll validation.\n\nByte-stable architecture\n------------------------\nWrapper: {WRAPPER_BYTES:,} / {WRAPPER_SHA}\nRelease adapter: {ADAPTER_BYTES} / {ADAPTER_SHA}\nRelease metadata: {METADATA_BYTES} / {METADATA_SHA}\nArchive inventory: {INVENTORY_BYTES:,} / {INVENTORY_SHA}\nAll byte-identical to v181 parent: yes\nRelease-specific diagnostic architecture changed files: 0\n\nRuntime\n-------\nAPP_VERSION / releaseVersion: v182\nCurrent adapter: runV182SelfCheck\nRetired adapters: 22 (runV160SelfCheck through runV181SelfCheck)\nDiagnostic archive: 58 / growth 0\nCanonical runtime: {snapshot['utf8_bytes']:,} bytes / SHA-256 {snapshot['sha256']}\nExcluded volatile field: FEQUEST_SELF_CHECK.checkedAt\n\nGenerated HTML\n--------------\n{len(prod_b):,} bytes / SHA-256 {sha_bytes(prod_b)}\n\nContracts\n---------\nCurrent 71/71\nBrowser UI 23/23 + required DOM\nCritical 56/56\nRelease sentinel 28/28\nCI 84/84\nLegacy 293 / raw22 / residual0\nQuestions 710 / unique710\nAnswers A178/B178/C177/D177\nCognitive 想起166/適用323/判断221\n''')
print('FEQUEST_V182_STEADY_STATE_OK wrapper-byte-stable=1 adapter-byte-stable=1 metadata-byte-stable=1 inventory-byte-stable=1 diagnostic-architecture-changed=0 retired-adapters=22 diagnostic-archive=58 archive-growth=0 snapshot=%s' % snapshot['sha256'])
print('FEQUEST_V182_ARCHIVE_BOUNDARY_OK diagnostic-archive=58 archive-growth=0 stable-archive-inventory=1 release-boundary-created=0 learning-archive=48 base-archive=2 deployed-regression=0 runtime=55525')
print('FEQUEST_V182_RUNTIME_OK current=71/71 stable=17 retired-fn=46 wrapper=6 retired-adapter=0 retired-adapter-inventory=22 adapter=1 data=6 backing=0 diagnostic-archive=58 archive-growth=0 stable-archive-inventory=1 release-boundary-created=0 learning-archive=48 base-archive=2 active-learning=1 active-base=1 stable-adapter=1 stable-release-metadata=1 versioned-adapter-source=0 diagnostic-architecture-changed=0 critical=56/56 release=28/28 ci=84 legacy=293 residual=0 production-legacy=0')
