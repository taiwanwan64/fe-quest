...
print('FEQUEST_V145_STATIC_DOM_OK 13/13')

scripts=re.findall(r'<script(?:\s[^>]*)?>(.*?)</script>',html,re.S|re.I)
js='\n'.join(s for s in scripts if s.strip() and not s.lstrip().startswith('{'))
Path('/tmp/fe-v145-inline.js').write_text(js,encoding='utf-8')
subprocess.run(['node','--check','/tmp/fe-v145-inline.js'],check=True)
...
'''
checks=r'''
if(APP_VERSION!=='v145') throw new Error(`runtime smoke: APP_VERSION ${APP_VERSION}`);
...
if(!sc||sc.architecture!=='v145-three-layer') throw new Error(`runtime smoke: architecture ${sc?.architecture}`);
if(!sc.ok||(sc.errors||[]).length!==0) throw new Error(`runtime smoke: active errors ${JSON.stringify(sc?.errors)}`);
if(sc.currentContract?.total!==23||sc.currentContract?.passed!==23||sc.currentContract?.failed!==0) throw new Error(`runtime smoke: current contract ${JSON.stringify(sc?.currentContract)}`);
if(sc.historicalDiagnostics?.residualActiveErrors?.length!==0) throw new Error(`runtime smoke: residual ${JSON.stringify(sc?.historicalDiagnostics?.residualActiveErrors)}`);
if(sc.historicalDiagnostics?.historicalMetadata?.length!==8) throw new Error(`runtime smoke: historical ${sc?.historicalDiagnostics?.historicalMetadata?.length}`);
if(sc.historicalDiagnostics?.supersededSemantic?.length!==1) throw new Error(`runtime smoke: semantic ${sc?.historicalDiagnostics?.supersededSemantic?.length}`);
if(sc.historicalDiagnostics?.legacyUiSignals?.length!==13) throw new Error(`runtime smoke: legacy ui ${sc?.historicalDiagnostics?.legacyUiSignals?.length}`);
if(sc.browserUiContract?.mode!=='deferred-to-browser-dom'||sc.browserUiContract?.total!==13||(sc.uiDeferredWarnings||[]).length!==13) throw new Error(`runtime smoke: ui deferred ${JSON.stringify(sc?.browserUiContract)}`);
if(sc.v145RuntimeMode!=='node-minimal-dom') throw new Error(`runtime smoke: mode ${sc?.v145RuntimeMode}`);
if(sc.v145DiagnosticContract?.activeErrorCount!==0||sc.v145DiagnosticContract?.compatibilityResidualCount!==0||sc.v145DiagnosticContract?.currentFailed!==0) throw new Error(`runtime smoke: diagnostic ${JSON.stringify(sc?.v145DiagnosticContract)}`);
console.log('FEQUEST_V145_RUNTIME_OK');
'''

runner=Path('/tmp/fe-v145-runtime.js'); runner.write_text(stub+'\n'+js+'\n'+checks,encoding='utf-8')
r=subprocess.run(['node',str(runner)],capture_output=True,text=True)
print(r.stdout)
print(r.stderr,file=sys.stderr)
if r.returncode!=0 or 'FEQUEST_V145_RUNTIME_OK' not in r.stdout:
