...
req('window.FEQUEST_SELF_CHECK=runV155SelfCheck();' in h,'v155-self-check-boot-missing')
req("const APP_VERSION = 'v155';" in h,'v155-version-missing')
manifest=Path('_site/manifest.webmanifest').read_text(encoding='utf-8')
sw=Path('_site/sw.js').read_text(encoding='utf-8')
req('"name": "FE QUEST v155"' in manifest,'manifest-version')
req("const APP_VERSION = 'v155';" in sw and "const CACHE_NAME = 'fe-quest-v155-1';" in sw,'service-worker-version')
...
req(hashlib.sha256(semantic_src.encode()).hexdigest()==semantic_spec['module_sha256']=='2493dafe66bfad28bde8f39e0dcd922d4ae1715b96007fc4cf259c748290447f','semantic-module-sha')
req(len(semantic_src.encode())==semantic_spec['module_utf8_bytes']==54897,'semantic-module-bytes')
...
print('FEQUEST_V155_FIXTURE_BOUNDARY_OK production=excluded fixture=293')
class N:
...
print('FEQUEST_V155_STATIC_DOM_OK 23/23 + required-dom')

scripts=re.findall(r'<script(?:\s[^>]*)?>(.*?)</script>',h,re.S|re.I)
js='\n'.join(s for s in scripts if s.strip() and not s.lstrip().startswith('{'))
Path('/tmp/fequest_v155_prod.js').write_text(js,encoding='utf-8')
subprocess.run(['node','--check','/tmp/fequest_v155_prod.js'],check=True)
...
prod_checks=r"""if(APP_VERSION!=='v155')throw Error('version');
...
if(stable.length!==17||retired.length!==46||stable.some(n=>typeof globalThis[n]!=='function')||retired.some(n=>typeof globalThis[n]!=='undefined'))throw Error('semantic diagnostic boundary');
if(typeof runV154SelfCheck!=='undefined'||typeof v154EvaluateCurrentContract!=='undefined'||typeof runV153SelfCheck!=='undefined')throw Error('pre-v155 runtime bundled');
...
if(!s||!s.ok||s.architecture!=='v155-stable-semantic-diagnostic-runtime'||s.currentContract.total!==71||s.currentContract.passed!==71||s.currentContract.failed)throw Error('self');
...
const b=s.semanticRuntimeBoundary;
if(!b||!b.ok||b.stable!==17||b.retired!==46||b.presentStable!==17||b.leakedRetired.length)throw Error('semantic boundary');
const lc=s.legacyClassification;
...
const ci=s.ciSentinelInventory,c=ci.bucketCounts;
...
const dc=s.declarativeCiCoverage;
...
console.log('FEQUEST_V155_PRODUCTION_RUNTIME_OK current=71/71 stable=17 retired=46 critical-map=56 release-map=28 ci=84 legacy-bundled=0');"""

prod_script=Path('/tmp/fequest_v155_prod_runtime.js')
...
req(z.returncode==0 and 'FEQUEST_V155_PRODUCTION_RUNTIME_OK' in z.stdout,'production-runtime')
...
fixture_release=fixture.replace(spec['release_shell_from'],template.replace('{{VERSION}}','v155'))
req(fixture_release!=fixture,'fixture-release-shell-adapter')
release_checks=r"""const retired=V155_SEMANTIC_DIAGNOSTIC_SPEC.retiredVersioned;
...
console.log('FEQUEST_V155_RELEASE_FIXTURE_OK diagnostic=46 critical=56/56 release=28/28 legacy=293 raw=22 residual=0');"""
legacy_script=Path('/tmp/fequest_v155_fixture_runtime.js')
import json
...
req(q.returncode==0 and 'FEQUEST_V155_RELEASE_FIXTURE_OK' in q.stdout,'fixture-runtime')

print('FEQUEST_V155_RUNTIME_OK current=71/71 stable=17 retired=46 critical=56/56 release=28/28 ci=84 legacy=293 residual=0 production-legacy=0')
