...
print('FEQUEST_V147_STATIC_DOM_OK 13/13')

scripts=re.findall(r'<script(?:\s[^>]*)?>(.*?)</script>',html,re.S|re.I)
js='\n'.join(s for s in scripts if s.strip() and not s.lstrip().startswith('{'))
Path('/tmp/fe-v147-inline.js').write_text(js,encoding='utf-8')
subprocess.run(['node','--check','/tmp/fe-v147-inline.js'],check=True)
...
'''
checks=r'''
if(APP_VERSION!=='v147')throw new Error(`version ${APP_VERSION}`);
...
if(!sc||sc.architecture!=='v147-safety-current-contract'||!sc.ok||(sc.errors||[]).length)throw new Error(`self check ${JSON.stringify(sc?.errors)}`);
if(sc.currentContract?.total!==53||sc.currentContract?.passed!==53||sc.currentContract?.failed!==0)throw new Error(`current ${JSON.stringify(sc?.currentContract)}`);
const inv=sc.v147MigrationInventory;
if(!inv||inv.inheritedCurrentContracts!==38||inv.newlyMigratedGroups!==15||inv.migratedLegacyAssertionEquivalents!==22||inv.legacyShadowAssertCalls!==293||inv.compatibilityResidualCount!==0||inv.browserUiContracts!==13)throw new Error(`inventory ${JSON.stringify(inv)}`);
const hist=sc.historicalDiagnostics;
if(hist?.rawErrorCount!==23||hist?.historicalMetadata?.length!==9||hist?.supersededSemantic?.length!==1||hist?.legacyUiSignals?.length!==13||hist?.residualActiveErrors?.length!==0)throw new Error(`historical ${JSON.stringify(hist)}`);
if(sc.browserUiContract?.mode!=='deferred-to-browser-dom'||sc.browserUiContract?.total!==13||(sc.uiDeferredWarnings||[]).length!==13)throw new Error('ui deferred');
if(sc.v147RuntimeMode!=='node-minimal-dom'||sc.v147DiagnosticContract?.activeErrorCount!==0)throw new Error('runtime mode');
console.log('FEQUEST_V147_RUNTIME_OK');
'''
runner=Path('/tmp/fe-v147-runtime.js');runner.write_text(stub+'\n'+js+'\n'+checks,encoding='utf-8')
r=subprocess.run(['node',str(runner)],capture_output=True,text=True)
print(r.stdout);print(r.stderr,file=sys.stderr)
if r.returncode!=0 or 'FEQUEST_V147_RUNTIME_OK' not in r.stdout:raise SystemExit(r.returncode or 1)
