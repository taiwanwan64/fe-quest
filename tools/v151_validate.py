...
print('FEQUEST_V151_STATIC_DOM_OK 23/23 + required-dom')

scripts=re.findall(r'<script(?:\s[^>]*)?>(.*?)</script>',h,re.S|re.I)
js='\n'.join(s for s in scripts if s.strip() and not s.lstrip().startswith('{'))
Path('/tmp/i.js').write_text(js,encoding='utf-8')
subprocess.run(['node','--check','/tmp/i.js'],check=True)
...
checks=r"""if(APP_VERSION!=='v151')throw Error('version');if(QUESTION_BANK.length!==710||new Set(QUESTION_BANK.map(q=>q.id)).size!==710)throw Error('questions');if([0,1,2,3].map(i=>QUESTION_BANK.filter(q=>q.a===i).length).join(',')!=='178,178,177,177')throw Error('answers');if(['想起','適用','判断'].map(k=>QUESTION_BANK.filter(q=>q.cognitiveLevel===k).length).join(',')!=='166,323,221')throw Error('cognitive');const s=FEQUEST_SELF_CHECK;if(!s||!s.ok||s.architecture!=='v151-full-ci-sentinel-declarative-contract'||s.currentContract.total!==67||s.currentContract.passed!==67||s.currentContract.failed)throw Error('self');if(s.historicalDiagnostics.mode!=='deferred-to-ci'||s.historicalDiagnostics.executed!==false)throw Error('defer');if(s.browserUiContract.mode!=='deferred-to-browser-dom'||s.browserUiContract.total!==23||s.uiDeferredWarnings.length!==23)throw Error('ui');const inv=s.v149LegacyInventory,b=inv.bucketCounts;if(inv.sourceAssertCalls!==293||inv.classifiedCalls!==293||inv.uniqueClassifiedCalls!==293||inv.duplicateCount||!inv.exactCoverage||!inv.declaredCountsOk||b.currentContractCovered!==147||b.browserContractCovered!==24||b.historicalMetadata!==38||b.ciOnlyLegacySentinels!==84)throw Error('legacy inventory');const ci=s.v150CiSentinelInventory,c=ci.bucketCounts;if(ci.classified!==84||ci.unique!==84||ci.duplicateCount||!ci.exactCoverage||!ci.declaredCountsOk||c.qualityCaps!==2||c.articleTeachingStructure!==4||c.criticalCurriculum!==56||c.assessmentStructure!==17||c.lessonPages!==1||c.referenceAnchors!==2||c.settingsHelper!==1||c.requiredDom!==1)throw Error('ci inventory');const dc=s.v151DeclarativeCiCoverage;if(!dc||dc.critical!==56||dc.release!==28||dc.total!==84||dc.unique!==84||dc.duplicateCount||!dc.exactCoverage)throw Error('declarative coverage');if(!s.v151ReleaseSentinelPolicy||s.v151ReleaseSentinelPolicy.productionExecutesFullAudit!==false||s.v151ReleaseSentinelPolicy.releaseCiExecutesFullAudit!==true||s.v151ReleaseSentinelPolicy.legacyAssertionsCovered!==28)throw Error('release policy');const cc=runV150CriticalCurriculumAudit();if(cc.total!==56||cc.passed!==56||cc.failed||cc.legacyAssertionsCovered!==56||!cc.coverageExact)throw Error('critical curriculum');const rs=runV151ReleaseSentinelAudit();if(rs.total!==28||rs.passed!==28||rs.failed||rs.legacyAssertionsCovered!==28||!rs.coverageExact)throw Error('release sentinels');const l=runV149LegacyShadowAudit();if(!l.executed||l.rawErrorCount!==22||l.historicalMetadata.length!==8||l.supersededSemantic.length!==1||l.legacyUiSignals.length!==13||l.residualActiveErrors.length)throw Error('legacy shadow');console.log('FEQUEST_V151_RUNTIME_OK current=67/67 critical=56/56 release=28/28 ci=84 legacy=293 residual=0');"""

r=Path('/tmp/r.js');r.write_text(stub+'\n'+js+'\n'+checks,encoding='utf-8')
z=subprocess.run(['node',str(r)],capture_output=True,text=True)
print(z.stdout)
print(z.stderr,file=sys.stderr)
raise SystemExit(0 if z.returncode==0 and 'FEQUEST_V151_RUNTIME_OK' in z.stdout else (z.returncode or 1))
