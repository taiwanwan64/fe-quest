...
def context():
    ...
    m = re.fullmatch(r'subject-a-mock-category-order-diversity-repair-(v(\d+))', branch)
...
function summarize(mode,runs,seedBase){
  ...
  const expectedCats=mockCategoryQuotas(bp.count);
  const exposure={}; let invalid=0,blueprintMismatch=0,strat16=0;
  const chapterCounts=[];
  for(let i=0;i<runs;i++){
    Math.random=seeded(seedBase+i*7919);
    const xs=buildMockQuestions(mode),ids=xs.map(q=>String(q.id||''));
    ...
    for(const q of xs){
      const id=String(q.id||''); exposure[id]=(exposure[id]||0)+1; if(id==='strat-16')strat16++;
      const m=String(q.coreTopicId||'').match(/(?:^|_)(\d{1,2})(?:_|$)/); if(m)chapters.add(Number(m[1]));
    }
    chapterCounts.push(chapters.size);
  }
  ...
  const stat=a=>{const s=[...a].sort((x,y)=>x-y);return {min:s[0],median:s[Math.floor(s.length/2)],max:s[s.length-1]};};
  return {mode,runs,invalidSessionCount:invalid,blueprintMismatch,strat16Pct:Math.round(strat16/runs*1000)/10,neverSelected:rates.filter(x=>x===0).length,exposurePct:{min:Math.round(rates[0]*10)/10,p50:Math.round(rates[Math.floor(rates.length*.5)]*10)/10,p90:Math.round(rates[Math.floor(rates.length*.9)]*10)/10,p95:Math.round(rates[Math.floor(rates.length*.95)]*10)/10,max:Math.round(rates[rates.length-1]*10)/10},topExposure:top,chaptersRepresented:stat(chapterCounts)};
}
const originalRandom=Math.random;
const summary={full:summarize('full',500,3061001),half:summarize('half',500,3069001)};
...
'''
    with tempfile.TemporaryDirectory() as td:
        ...
        m = re.search(r'__V306__([A-Za-z0-9+/=]+)', z.stdout)
        req(m is not None, 'runtime marker missing')
        return json.loads(base64.b64decode(m.group(1)))


version, previous = context()
req((version, previous) == ('v306','v305'), 'expects v306 over v305')
parent = subprocess.check_output(['git','rev-parse','origin/main'], text=True).strip()

v305p = Path('_regression/subject-a-mock-selection-order-simulation-v305.fixture.json')
req(v305p.exists(), 'v305 fixture missing')
v305 = json.loads(v305p.read_text())
req(v305.get('result') == 'PASS — SELECTION-ORDER ALTERNATIVES SIMULATED', 'v305 fixture result')

source_allowed = {
    'app/subject-a-mock-selection-diversity-overrides-v306.txt',
    'index.html',
    '.github/subject-a-mock-category-order-diversity-repair/prepare_reference.py',
    '.github/subject-a-mock-category-order-diversity-repair/validate_repair.py',
    '.github/workflows/subject-a-mock-category-order-diversity-repair.yml',
}
generated_allowed = {
    'manifest.webmanifest','sw.js',
    '_regression/subject-a-mock-category-order-diversity-repair-v306.fixture.json',
    'audits/SUBJECT_A_MOCK_CATEGORY_ORDER_DIVERSITY_REPAIR_v306.txt',
}
...
spec = cand['spec'] or {}
req(spec.get('policy') == 'randomize-category-traversal-only', 'repair policy marker')
for key in ['categoryQuotaChanged','difficultyQuotaChanged','cognitiveTargetChanged','difficultyTraversalChanged','conceptCapChanged','noveltyOrderChanged','questionContentChanged','profileSchemaMigrationRequired','remoteTelemetry']:
    req(spec.get(key) is False, 'unexpected policy change '+key)
req('categoryOrder=shuffled([...MOCK_CATEGORIES])' in cand['fn'].replace(' ',''), 'candidate does not randomize category traversal')
req('MOCK_DIFFICULTY_LEVELS.forEach' in cand['fn'], 'difficulty traversal contract missing')
req('categoryOrder=shuffled([...MOCK_CATEGORIES])' not in par['fn'].replace(' ',''), 'parent unexpectedly already randomized')

for mode in ['full','half']:
    c = cand['summary'][mode]; p = par['summary'][mode]
...
summary = {
    'sourceDecision':'v305 showed category traversal is the dominant cause of the fixed strat-16 slot; apply the smaller category-only change rather than randomizing difficulty traversal too',
    'parent':par['summary'],
    'candidate':cand['summary'],
    'fullStrat16ReductionPctPoints':round(par['summary']['full']['strat16Pct']-cand['summary']['full']['strat16Pct'],1),
    'halfStrat16ReductionPctPoints':round(par['summary']['half']['strat16Pct']-cand['summary']['half']['strat16Pct'],1),
    'preserved':['60/30 question counts','category quotas','difficulty quotas','cognitive targets','concept caps','novelty ordering','question content','profile schema','Subject B semantics'],
}
fixture = {
    'version':version,'previous':previous,'parent':parent,
    'result':'PASS — CATEGORY-ORDER DIVERSITY REPAIR VALIDATED',
    'summary':summary,'semanticOK':True,'candidateReferenceSixFileByteEquality':True,
}
Path('_regression').mkdir(exist_ok=True)
Path('_regression/subject-a-mock-category-order-diversity-repair-v306.fixture.json').write_text(json.dumps(fixture,ensure_ascii=False,indent=2)+'\n')

audit = f'''FE QUEST v306 — Subject A Mock Category-Order Diversity Repair\n=================================================================\n\nResult\n------\nPASS — CATEGORY-ORDER DIVERSITY REPAIR VALIDATED\nPrevious release: v305\nSource main: {parent}\nLearner-facing change: Subject A mock category traversal is randomized before the existing greedy selector runs.\n\nWhy this repair\n---------------\nv305 proved that the fixed category traversal order—not the cognitive label itself—caused strat-16 to appear in every full and half mock. Randomizing category order reduced that fixed exposure while preserving all mock blueprints. v306 therefore applies only the smaller category-order change and leaves difficulty traversal fixed.\n\n500-session runtime comparison\n------------------------------\n{json.dumps(summary,ensure_ascii=False,indent=2)}\n\nRegression\n----------\nQUESTION_BANK content and answer keys: unchanged.\nMOCK_BLUEPRINTS: unchanged.\nCategory, difficulty and cognitive counts: exact in every simulated full/half session.\nDuplicate question IDs: none.\nNo new question exceeds 60% exposure in either mode.\nSubject B semantic diagnostics: OK.\nCandidate/approved-reference six-file byte equality: yes.\n\nDecision\n--------\nKeep the v306 category-only traversal repair. Do not also randomize difficulty traversal: v305 showed little additional benefit, while category-only is the narrower behavior change and removes the pathological fixed slot.\n'''
Path('audits').mkdir(exist_ok=True)
Path('audits/SUBJECT_A_MOCK_CATEGORY_ORDER_DIVERSITY_REPAIR_v306.txt').write_text(audit)
print(audit)
