...
def dump_runtime(html_path):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        m=re.search(r'__FEQ_QB__([A-Za-z0-9+/=]+)',z.stdout); req(m is not None,'question dump marker missing')
        return json.loads(base64.b64decode(m.group(1)).decode())

branch,version,number,previous=context()
manifest_path=Path(f'_release/content-change-{version}.json'); req(manifest_path.exists(),'content manifest missing')
...
allowed_ids=manifest.get('allowed_question_ids',[]); allowed_fields=set(manifest.get('allowed_changed_question_fields',[])); marker=manifest.get('quality_audit_marker')
...
cq=candidate['questions']; pq=parent_rt['questions']; req(len(cq)==710 and len(pq)==710,'question count')
cm={q['id']:q for q in cq}; pm={q['id']:q for q in pq}; req(len(cm)==710 and len(pm)==710 and set(cm)==set(pm),'question id set')
changed=[]; changed_fields={}
for qid in cm:
    if cm[qid]!=pm[qid]:
        changed.append(qid); changed_fields[qid]=sorted(k for k in set(cm[qid])|set(pm[qid]) if cm[qid].get(k)!=pm[qid].get(k))
...
for qid in allowed_ids:
    ...
    a,b=pm[qid],cm[qid]
    for k in ['id','a','cognitiveLevel','cat','concept']: req(a.get(k)==b.get(k),'preserved question field drift '+qid+' '+k)
    req(len(b.get('options',[]))==4 and len(b.get('choiceExps',[]))==4,'four-choice structure '+qid)
    req(b.get('qualityAudit')==marker,'quality audit marker '+qid)
