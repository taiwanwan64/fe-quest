...
def context():
    ...
    req(m is not None,'full data reset branch must match full-data-reset-vNNN')
...
req((version,previous)==('v333','v332'),'v333 full reset context expected')
archive_main('_reference')
archive_main('_parent_reference')

# Approved target: untouched main plus only the full-reset override and release shell.
for rel in ['app/learning-data-reset-overrides-v333.txt','index.html','manifest.webmanifest','sw.js']:
...
print(f'FEQUEST_V333_FULL_RESET_REFERENCE_READY version={version} previous={previous} parent='+subprocess.check_output(['git','rev-parse','origin/main'],text=True).strip())
