...
PREVIOUS = "v353"
TARGET = "v354"
JS_SOURCE = ROOT / "app/database-normalization-diagram-v354.js"
CSS_SOURCE = ROOT / "app/database-normalization-diagram-v354.css"
CSS_MARKER = "v354: database normalization before-and-after lesson diagram"
...
if target["css"].read_text().count(CSS_MARKER) != 1:
    raise AssertionError("v354 database normalization CSS must be materialized exactly once")

js = target["js"].read_text()
source = JS_SOURCE.read_text().strip()
function_anchor = "function coreTopicArticleView(id){"
if "function coreTopicDatabaseDiagramViewV354(id)" not in js:
...
if new_mount not in js:
    if old_mount not in js:
        raise AssertionError("v353 lesson diagram mount anchor missing")
    js = js.replace(old_mount, new_mount, 1)
if js.count("function coreTopicDatabaseDiagramViewV354(id)") != 1:
    raise AssertionError("v354 database diagram renderer must be unique")
if js.count("${coreTopicDatabaseDiagramViewV354(id)}") != 1:
    raise AssertionError("v354 database diagram mount must be unique")
...
manifest["databaseNormalizationDiagram"] = {
    "version": TARGET,
    "scope": "core-09-03-database-normalization-diagram",
...
print(f"FEQUEST_V354_MATERIALIZED already={int(result['already_materialized'])} css-marker=1 diagram=1")
