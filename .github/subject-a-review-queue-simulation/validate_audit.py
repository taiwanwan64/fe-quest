...
const process=dueQuestions().slice(0,10);
...
const configuredIds=[...today,...tomorrow,...future].map(q=>q.id);
...
d0=cand['day0'];d1=cand['day1Before'];da=cand['day1After'];req(len(d0['active'])==3,'expected 3 active journeys');req(len(d0['due'])==37,'active journeys should be removed from generic due queue');req(d0['workload']==len(d0['actionable'])+len(d0['due']),'workload formula mismatch');req(d0['workload']==40,'day0 workload should represent 40 unique due items');req([x['count'] for x in d0['forecast']]==[40,8,0,5],'day0 forecast mismatch')
req(len(d1['due'])==45,'overdue items were dropped or tomorrow due set missing');req(d1['workload']==48,'day1 workload should preserve all 48 due/recovery items');req(len(cand['processed'])==10,'processing sample');req(len(da['due'])==35,'processed due items did not leave generic queue');req(da['workload']==38,'post-processing workload mismatch');req(len(cand['stateDueNow'])==38,'underlying due-state count mismatch');req(len(set(d0['due']))==len(d0['due']) and len(set(d1['due']))==len(d1['due']),'duplicate generic due IDs');req(len(d0['top'])==3,'home candidate display should remain a 3-item preview')
...
summary={
 ...
 'day0':{'active':len(d0['active']),'actionable':len(d0['actionable']),'genericDue':len(d0['due']),'workload':d0['workload'],'forecast':d0['forecast'],'topPreview':d0['top'],'alloc60':d0['alloc60'],'alloc90':d0['alloc90']},
 'day1WithoutCompletion':{'genericDue':len(d1['due']),'workload':d1['workload']},
 'day1AfterTenCorrectReviews':{'processed':len(cand['processed']),'genericDue':len(da['due']),'workload':da['workload'],'underlyingDueStates':len(cand['stateDueNow'])},
 'interpretation':'The queue preserves backlog instead of silently capping or dropping overdue items. Guided review journeys are excluded from the generic due list and then added once through reviewWorkloadCount, so the 3 overlapping journey items are not double-counted. When a day is skipped, overdue work remains present on the next day; after 10 clean reviews, exactly those 10 leave the current queue through their new future due dates. The home top-review list is only a 3-item preview, not a queue cap. One remaining design question is capacity: normal 60/90-minute plans allocate 10/15 minutes to review regardless of whether workload is small or very large. That is not itself a correctness bug, but it warrants inspection of the actual review-task launch size before changing allocation.',
 'decision':'QUEUE INTEGRITY PASS — INSPECT REVIEW TASK CAPACITY BEFORE ANY ALLOCATION CHANGE'
}
...
audit=f'''FE QUEST v315 — Subject A Review Queue Multi-Day Simulation\n=============================================================\n\nResult\n------\nPASS — SUBJECT A REVIEW QUEUE INTEGRITY COHERENT\nPrevious release: v314\nSource main: {parent}\nLearner-facing change: none\n\nMethod\n------\nSeed a controlled profile with 40 questions due today, 8 tomorrow, 5 three days later, and 3 guided review journeys overlapping today's due set. Observe generic due selection, workload aggregation, forecast, skipped-day carryover, and the result of completing 10 clean reviews.\n\nSummary\n-------\n{json.dumps(summary,ensure_ascii=False,indent=2)}\n\nRegression\n----------\nLearner-facing behavior and review queue functions are unchanged from v314.\nSubject B semantic diagnostics: OK.\nCandidate/mechanical-reference six-file equality: yes.\n\nDecision\n--------\nNo queue-loss or duplicate-count repair is needed. Before deciding whether large backlogs deserve more of the daily study budget, inspect the production action launched by the review task and determine how many questions/minutes it actually consumes.\n''';Path('audits').mkdir(exist_ok=True);Path('audits/SUBJECT_A_REVIEW_QUEUE_SIMULATION_v315.txt').write_text(audit);print(audit)
