...
const seed=(q,due,last)=>{const st=profile.qStats[q.id];Object.assign(st,{attempts:1,correct:1,streak:1,due,last,stability:3,lapses:0,reviews:1,avgSeconds:60,timedAnswers:1,lastReviewDate:last,lastQuality:4});};
...
today.forEach(q=>seed(q,'2026-05-01','2026-04-28'));tomorrow.forEach(q=>seed(q,'2026-05-02','2026-04-29'));future.forEach(q=>seed(q,'2026-05-04','2026-05-01'));
...
const stateDueNow=configuredIds.filter(id=>{const st=profile.qStats[id];return st.attempts>0&&isDue(st);});
const sources={reviewWorkloadCount:String(reviewWorkloadCount),reviewForecast:String(reviewForecast),dueQuestions:String(dueQuestions),buildTodayTasks:String(buildTodayTasks),taskAllocation:String(taskAllocation),activeReviewJourneys:String(activeReviewJourneys),actionableReviewJourneys:String(actionableReviewJourneys),isDue:String(isDue)};
