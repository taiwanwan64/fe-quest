...
# Production adapters intentionally resolve these names from the classic browser global scope.
# Define the mock persistence globals before loading those modules so Node mirrors that binding model.
prelude=r'''
var profile={profileSchemaVersion:5,profileMeta:{createdAt:'2026-08-22T00:00:00Z',updatedAt:'2026-08-22T00:00:00Z',revision:1,lastWriterId:'writer-a'},xp:10,sessions:[],lessonProgress:{},bProgress:{},qStats:{}};
var checksum='fnv1a32:11111111';var originalWrites=0;var recovery=0;
...
tail=r'''
const cases=[];const ok=(name,cond)=>cases.push({name,pass:Boolean(cond)});const api=FEQUEST_CLOUD_RUNTIME_V342;
const defaultRuntime=api.createCloudRuntime();ok('default public config keeps cloud runtime disabled',defaultRuntime.ok&&defaultRuntime.status==='disabled');
let secretRejected=false;try{api.validateEnabledConfig({enabled:true,provider:'supabase',url:'https://x.supabase.co',publishableKey:'service_role_abcdefghijklmnopqrstuvwxyz',redirectTo:'https://example.com/'})}catch(e){secretRejected=true}ok('secret service-role credential is rejected before assembly',secretRejected);
let urlRejected=false;try{api.validateEnabledConfig({enabled:true,provider:'supabase',url:'http://x.supabase.co',publishableKey:'public-key-abcdefghijklmnopqrstuvwxyz',redirectTo:'https://example.com/'})}catch(e){urlRejected=true}ok('enabled config requires HTTPS project URL',urlRejected);
...
const userId='11111111-1111-4111-8111-111111111111';let session={user:{id:userId,email:'learner@example.com'},access_token:'access-token-abcdefghijklmnopqrstuvwxyz',expires_at:9999999999};let authListener=null,clientOptions=null,fetchCalls=[],unsubscribed=0;
...
const runtime=api.createCloudRuntime({config:enabled,createClient,storage,fetchImpl,document:null,confirm:()=>true,warn:()=>{}});ok('valid public config assembles runtime without starting it',runtime.ok&&runtime.status==='ready'&&fetchCalls.length===0);
...
const networkBeforeSave=fetchCalls.length;writeCurrentProfile(profile);const afterSaveState=runtime.snapshot().sync;ok('post-local-commit bridge queues newer local state without network',afterSaveState.pending&&afterSaveState.pending.profileRevision===profile.profileMeta.revision&&fetchCalls.length===networkBeforeSave&&originalWrites>=1);
...
for forbidden in ['service_role_', 'sb_secret_']:req(forbidden not in config,f'forbidden secret marker in public config: {forbidden}')
