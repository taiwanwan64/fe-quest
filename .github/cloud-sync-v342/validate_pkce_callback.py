...
(async()=>{
  ...
  ok('non-email callback type is ignored',A.parseEmailTokenHashCallback('https://example.test/?token_hash=x&type=recovery')===null);
  ok('missing token hash is ignored',A.parseEmailTokenHashCallback('https://example.test/?type=email')===null);
  ...
  let current=null,verifyArgs=null,verifyCount=0,getSessionCount=0,replaceArgs=[],authErrors=[];
