...
PLAN=Path('FE_QUEST_DEVELOPMENT_PLAN.md')
...
# Discover persistence/sync-adjacent functions from the real production asset.
...
# Storage constants and literal keys.
...
# IndexedDB database/store literal inventory.
idb_literals=sorted(set(re.findall(r'indexedDB\.open\(\s*[\'\"]([^\'\"]+)',js)))

# Runtime snapshot from the exact production JS.
...
const __v342out={
 ...
 profileSnapshot:__v342safe(()=>JSON.parse(JSON.stringify(profile))),
...
# Static contracts relevant to cloud design.
...
# Classify likely sync write boundary from source evidence.
...
out={
 'audit':'FE QUEST v342 cloud-sync contract discovery',
 'sourceVersion':runtime['version'],
 'learnerFacingChange':'none',
 'productionAsset':{'path':str(APP),'bytes':APP.stat().st_size,'sha256':hashlib.sha256(APP.read_bytes()).hexdigest()},
 'runtime':runtime,
 'persistenceFunctionInventory':fn,
 'storageLiteralKeys':key_literals,
 'storageKeyConstants':key_constants,
 'indexedDBLiteralNames':idb_literals,
 'metadataTokenCounts':meta_tokens,
 'networkTokenCounts':network_tokens,
 'writeBoundaryEvidence':write_boundary,
 'splitReleaseTooling':split_release,
 'designConstraints':[
   'local profile remains usable with no account and no network',
   'cloud write must happen only after local atomic save succeeds',
   'revision/updatedAt/lastWriterId must not be bypassed',
   'recovery center and JSON export remain independent escape hatches',
   'old cloud data must never silently overwrite a newer local revision',
   'provider SDK/network failure must not block saveProfile or app startup',
...
summary={
 'schema':runtime.get('profileSchemaVersion'),'profileMeta':runtime.get('profileMeta'),
 'profileKeyCount':len(runtime.get('profileKeys') or []),'settingsKeys':runtime.get('settingsKeys'),
 'persistenceFunctions':len(fn),'literalStorageKeys':len(key_literals),'indexedDBNames':idb_literals,
