...
cand,par=runtime('_site/index.html'),runtime('_site_parent/index.html');req(cand['v']=='v322' and par['v']=='v321','versions');req({k:v for k,v in cand.items() if k not in ('v','sem')}=={k:v for k,v in par.items() if k not in ('v','sem')},'audit-only runtime drift');req(cand['sem'].get('ok') is True and par['sem'].get('ok') is True,'semantic')
...
# A taper-specific minute cap is evidence only here; absence is a finding, not an automatic formula rewrite.
