...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.split('?', 1)[0] in ('/', '/index.html'):
            body = (ROOT / 'app/base-shell-v360.html').read_bytes()
...
def run_case(pw, base, name, engine, viewport, mobile=False):
    ...
    try:
        ...
        check('HTTP 200 and v360 runtime', response.status == 200 and page.evaluate('APP_VERSION') == 'v360')
        ...
        check('no page errors or recovery screen', not errors and page.locator('#fequestAssetRecoveryV360').count() == 0)
...
def main():
    ...
    thread.start()
    base = os.environ.get('V360_BASE_URL') or f'http://127.0.0.1:{server.server_address[1]}/'
    try:
    ...
    report = {'name':'v360-stack-queue-diagrams','baseUrl':base,'cases':cases,'result':'PASS' if all(c['pass'] for c in cases) else 'FAIL'}
    (OUT / 'result.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n')
    print(json.dumps(report,ensure_ascii=False,indent=2))
    if report['result'] != 'PASS':
        return 1
    print(f'PASS — V360 STACK QUEUE BROWSER {len(cases)}/{len(cases)}')
    return 0
