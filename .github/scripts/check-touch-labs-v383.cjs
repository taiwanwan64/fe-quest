...
labs.close();
labs.open('tcpudp');
assert.match(dialog.querySelector('.touch-lab-activity').innerHTML,/途中でパケットを失う/);
const drop=new Node('button');drop.dataset.transportDrop='';
dialog.listeners.click({target:drop});
const send=new Node('button');send.dataset.transportSend='';
dialog.listeners.click({target:send});
assert.match(dialog.querySelector('.touch-lab-activity').innerHTML,/パケット消失を検出して再送/);
labs.close();
