...
const IPA92_EXTENSION_TOTAL=IPA92_V1_V6_TOTAL+IPA92_V7_TOTAL+IPA92_V8_TOTAL+IPA92_V9_TOTAL+IPA92_V10_TOTAL+IPA92_V11_TOTAL+IPA92_V12_TOTAL+IPA92_V13_TOTAL;
...
async function loadCatalog(){
  if(!catalogPromise){
    catalogPromise=Promise.all([
      fetchJson(BASE_CATALOG_URL),fetchJson(IPA92_V1_V6_CATALOG_URL),fetchJson(IPA92_V7_CATALOG_URL),fetchJson(IPA92_V8_CATALOG_URL),fetchJson(IPA92_V9_CATALOG_URL),fetchJson(IPA92_V10_CATALOG_URL),fetchJson(IPA92_V11_CATALOG_URL),fetchJson(IPA92_V12_CATALOG_URL),fetchJson(IPA92_V13_CATALOG_URL)
    ]).then(([base,legacy,v7,v8,v9,v10,v11,v12,v13])=>{
      ...
      if(
        v10?.version!==IPA92_V10_CATALOG_VERSION||v10?.contentVersion!==IPA92_V10_CONTENT_VERSION||
        !sameStringArray(v10?.contentVersions,[IPA92_V10_CONTENT_VERSION])||!Array.isArray(v10.items)||v10.items.length!==IPA92_V10_TOTAL
      )throw new Error('ipa92_v10_question_catalog_invalid');
      if(!v10.items.every(safeIpa92CatalogItem)||v10.items.some(item=>item.qualityAudit!=='ipa92-original-v10'))throw new Error('ipa92_v10_question_catalog_invalid');
      if(
        v11?.version!==IPA92_V11_CATALOG_VERSION||v11?.contentVersion!==IPA92_V11_CONTENT_VERSION||
        !sameStringArray(v11?.contentVersions,[IPA92_V11_CONTENT_VERSION])||!Array.isArray(v11.items)||v11.items.length!==IPA92_V11_TOTAL
      )throw new Error('ipa92_v11_question_catalog_invalid');
      if(!v11.items.every(safeIpa92CatalogItem)||v11.items.some(item=>item.qualityAudit!=='ipa92-original-v11'))throw new Error('ipa92_v11_question_catalog_invalid');
      if(
        v12?.version!==IPA92_V12_CATALOG_VERSION||v12?.contentVersion!==IPA92_V12_CONTENT_VERSION||
        !sameStringArray(v12?.contentVersions,[IPA92_V12_CONTENT_VERSION])||!Array.isArray(v12.items)||v12.items.length!==IPA92_V12_TOTAL
      )throw new Error('ipa92_v12_question_catalog_invalid');
      if(!v12.items.every(safeIpa92CatalogItem)||v12.items.some(item=>item.qualityAudit!=='ipa92-original-v12'))throw new Error('ipa92_v12_question_catalog_invalid');
      if(
        v13?.version!==IPA92_V13_CATALOG_VERSION||v13?.contentVersion!==IPA92_V13_CONTENT_VERSION||
        !sameStringArray(v13?.contentVersions,[IPA92_V13_CONTENT_VERSION])||!Array.isArray(v13.items)||v13.items.length!==IPA92_V13_TOTAL
      )throw new Error('ipa92_v13_question_catalog_invalid');
      if(!v13.items.every(safeIpa92CatalogItem)||v13.items.some(item=>item.qualityAudit!=='ipa92-original-v13'))throw new Error('ipa92_v13_question_catalog_invalid');

      const baseIds=base.items.map(item=>item.id);
      const extensionItems=[...legacy.items,...v7.items,...v8.items,...v9.items,...v10.items,...v11.items,...v12.items,...v13.items];
      ...
      return Object.freeze({
        ...base,
        version:`${BASE_CATALOG_VERSION}+ipa92-catalog-v1-v13`,
        extensionContentVersion:'ipa92-questions-v1-v13',
        extensionContentVersions:Object.freeze([...IPA92_V1_V6_CONTENT_VERSIONS,IPA92_V7_CONTENT_VERSION,IPA92_V8_CONTENT_VERSION,IPA92_V9_CONTENT_VERSION,IPA92_V10_CONTENT_VERSION,IPA92_V11_CONTENT_VERSION,IPA92_V12_CONTENT_VERSION,IPA92_V13_CONTENT_VERSION]),
...
window.FEQUEST_PROTECTED_CONTENT=Object.freeze({
  version:'v376-provider-10-ipa92-v1-v13',maxBatch:MAX_BATCH,maxCache:MAX_CACHE,catalogTotal:MERGED_CATALOG_TOTAL,
